...
#include "rigid_world_phase9_decode.hpp"
#include "rigid_world_phase10_decode.hpp"
inline RigidWorldRequest decode(std::string_view record) {
  ...
  if (raw_timelines.is_array() && raw_timelines.size() == 19) {
    auto phase10_timelines = decode_phase10_timelines(raw_timelines);
    auto phase10_stripped = strip_phase10_for_phase9(root);
    auto phase9_timelines = decode_phase9_timelines(
        phase10_stripped["scenario"]["timelines"]);
    auto legacy_root = strip_phase9_for_legacy_decode(phase10_stripped);
    ...
    request.phase8_timelines = decode_phase8_timelines(
        strip_phase9_for_legacy_decode(phase10_stripped)["scenario"]["timelines"]);
    request.phase9_timelines = std::move(phase9_timelines);
    request.phase10_timelines = std::move(phase10_timelines);
    return request;
  }

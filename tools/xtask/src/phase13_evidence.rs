...
impl ReplayEvidenceRecord {
    fn validate(&self) -> Result<(), Phase13EvidenceError> {
        let valid = self.schema_version == 1
            ...
            && self.diagnosis.current_schema.catalog_schema_version == 1
            && self.diagnosis.current_schema.checkpoint_schema_version == 1
            && self.diagnosis.current_schema.projection_version == "expanded_checkpoint_v1"
            && self.diagnosis.reviewed_schema.catalog_schema_version == 1
            && self.diagnosis.reviewed_schema.checkpoint_schema_version == 1
            && self.diagnosis.reviewed_schema.projection_version == "legacy_physics_v1"
...
fn replay_record(replay: &ReplayOutput) -> Result<ReplayEvidenceRecord, Phase13EvidenceError> {
...
fn live_check(repository_root: &Path) -> Result<(), Phase13EvidenceError> {
    let tracked_path =
    ...
    if let Err(mismatch) = compare_live_replay_records(&reviewed, &current) {
...
fn persist_acquisition_failure(
    repository_root: &Path,
    persist_failures: bool,
...
fn persist_live_acquisition_failure(
    repository_root: &Path,

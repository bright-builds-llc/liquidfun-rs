...
## Canonical follow-up — 2026-09-16

Source `91ac59b3068ec4c5929833b16413e478854d5fac` passed both canonical Linux and ASan/UBSan Phase 11 lanes in run `35140576574`, attempt 1. The six canonical and three sanitizer comparison commands passed, including the original failing particle scenario. Provider archives and raw logs are retained under `target/phase15-catalog-repair/canonical-check-01/`. Independent source review, including the recycled-handle finding and correction, passed before that source was published.

This confirms the source repair under canonical tooling; it is not final-candidate release evidence. The legacy exact-reference semantic-result topology incompatibility discovered during independent archive validation is repaired separately and does not alter the physics adapter or the retained producer bytes.

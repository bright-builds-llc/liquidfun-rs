...
- [ ] **WASM-01**: A visitor can run and visibly step a particle/rigid-body scene using this repository's Rust engine compiled to WebAssembly, without a C++ runtime or external JavaScript physics engine.
- [ ] **WASM-02**: A contributor can reproducibly build the local JS/TypeScript WASM package and frontend from a clean checkout with documented commands and pinned tool inputs; ordinary native Cargo consumers remain independent of browser tooling.
- [ ] **WASM-03**: The browser renderer receives bulk owned frame data through a typed interface without per-particle JS/Rust calls or exposed raw engine pointers.
...
- [ ] **WEB-02**: A visitor can share and reload a stable demo URL under the repository's GitHub Pages path; an unknown scene identifier returns a useful catalog/fallback view.
...
- [ ] **WEB-06**: WASM loading and simulation startup show visible progress or loading status, useful failure feedback, and a working retry/reset path instead of a blank canvas.
...
| --- | --- | --- |
| WASM-01 | Phase 16 | Pending |
| WASM-02 | Phase 16 | Pending |
| WASM-03 | Phase 16 | Pending |

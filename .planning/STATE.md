...
- [Phase 07]: Keep tree and proxy identities behind crate-private BroadPhase adapters while public callbacks receive only FixtureId and ChildIndex semantics.
- [Phase 07]: Represent callback clipping with a normalized RayCastFraction newtype, then validate it again against the current narrowed traversal interval before application.
- [Phase 07]: Map both Ignore and Continue to interval-preserving private traversal while retaining distinct public meanings and keeping callback order explicitly unspecified.
...
- [Phase 07]: Use pinned 64-body and 32-contact TOI island capacities with source-ordered seed-body adjacency expansion. — Fixed private bounds preserve upstream traversal behavior and prevent adversarial contact graphs from creating unbounded solver work.
...
- [Phase 07]: Canonicalize query occurrences and non-terminated ray hits within the exact final interval as multisets only in the evidence layer. — Production traversal remains source-faithful while arbitrary valid clips discard only callback-order-dependent history above the final interval.
...
- [Phase 10]: Execute the validated particle manifest through one crate-private executor shared by production and exact trace tests. — One dispatch function prevents production and verification from drifting into separate order authorities.
